...
{  
	$_xmlHttpRequest();
	xmlHTTP.open("POST","bossOrder.php", true);		
	...
	xmlHTTP.onreadystatechange=function check_user(){
        if(xmlHTTP.readyState == 4){
            if(xmlHTTP.status == 200){
                ...
                ob = JSON.parse(str);
                console.log(ob);
                document.getElementById("userName").innerHTML = "老闆";
                        ...
                        if (ob[i + 1].orderId != oldOrderId || i == (ob.length - 1)) {  //判斷什麼時候要印出訂單編號
                            ...
                            switch (ob[i].states) {
                                ...
                                case '0':
                                    ...
                                    newButton.setAttribute("onclick","orderAccept('"+ob[i].orderId+"','"+completedButtonId+"','"+ob[i].userId+"')");
                                    newButton.innerHTML = "接受訂單";
                                    break;
                                ...
                                case '1':
                                    ...
                                    var completedButtonId = "completedButton" + i;
                                    newButton.setAttribute("id",""+completedButtonId+"");
                                    newButton.setAttribute("onclick","orderSend('"+ob[i].orderId+"','"+completedButtonId+"','"+ob[i].userId+"')");
                                    newButton.innerHTML = "寄出訂單";
                                    break;
                                ...
                                case '2':
                                    ...
                                    newButton.setAttribute("type", "button");
                                    newButton.setAttribute("disabled", true);
                                    newButton.innerHTML = "訂單已抵達";
                                    break;
                                case '3':
                                    newButton.setAttribute("class", "btn btn-secondary");
                                    newButton.setAttribute("type", "button");
                                    newButton.setAttribute("disabled", true);
                                    newButton.innerHTML = "訂單已完成";
                                    break;
                            }
...
function orderAccept(orderId,completedButtonId,userId) {

    $_xmlHttpRequest();
    var completedButton = document.getElementById(""+completedButtonId+"");
    xmlHTTP.open("POST", "orderAccept.php", true);
    ...
    xmlHTTP.onreadystatechange = function check_user() {
        if(xmlHTTP.readyState == 4){
            if(xmlHTTP.status == 200){
                ...
                completedButton.innerHTML = "寄出訂單";
            }
        }
    }
    xmlHTTP.send(data);
...
function orderSend(orderId,completedButtonId,userId) {

    $_xmlHttpRequest();
    var completedButton = document.getElementById(""+completedButtonId+"");
    xmlHTTP.open("POST", "orderSend.php", true);
    var data = 'orderId='+orderId+'&userId='+userId;
    console.log(orderId);
    xmlHTTP.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlHTTP.onreadystatechange = function check_user() {
        if(xmlHTTP.readyState == 4){
            if(xmlHTTP.status == 200){
                completedButton.setAttribute("class","btn btn-secondary");
                completedButton.innerHTML = "訂單已抵達";
